"""
main.py
Punto de entrada de SQL-SafeBridge API.

Verifica e instala automáticamente las dependencias requeridas antes de
iniciar el servidor API para Electron.
"""
import importlib
import subprocess
import sys
...
def main() -> None:
    """Función principal de arranque del servidor API."""
    _ensure_dependencies()

    # Importaciones después de garantizar que las dependencias existen
    from api.server import create_app
    import shared.config as config
    
    # Crear y ejecutar servidor Flask
    app = create_app()
    
    host = getattr(config, 'API_HOST', '127.0.0.1')
    port = getattr(config, 'API_PORT', 5000)
    debug = getattr(config, 'API_DEBUG', False)
    
    print(f"[SQL-SafeBridge] API iniciada en http://{host}:{port}")
    print("[SQL-SafeBridge] Esperando conexiones de Electron...")
